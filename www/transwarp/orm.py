...
class Field(object):
    ...
    def __init__(self,**kw):
        ...
        self.insertable = kw.get('insterable',True)
        self.ddl = kw.get('ddl','')
...
def _gen_sql(table_name,mapping):
    ...
    for f in sorted(mapping.value(),lambda x,y: cmp(x._order,y._order)):
...
class ModelMetaclass(type):
    ...
    def __new__(cls, name,bases,attrs):
        ...
        if not primary_key:
            raise  TypeError('Primary key not found in class: %s ' % name)
        ...
        attrs['sql'] = lambda self: _gen_sql(attrs['__table__',mapping])
...
if __name__ == '__main__':
    ...
    db.create_engine('root','123456','test',host='192.168.3.96')
